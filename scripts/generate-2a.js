...
function emptyLines(n=1) {
  return Array.from({ length:n }, ()=>new Paragraph({ spacing:{ before:40, after:40 }, children:[new TextRun({ text:"" })] }));
}
...
function titleBlock(subtitle, subtitle2) {
  return new Table({
    width: { size:CONTENT_W, type:WidthType.DXA },
    ...
    rows: [new TableRow({ children: [
      new TableCell({
        ...
        children: [
          new Paragraph({ children:[new TextRun({ text:subtitle, font:FONT, size:28, bold:true, color:C.HEADER_FG })] }),
          new Paragraph({ children:[new TextRun({ text:subtitle2, font:FONT, size:22, color:"CCDDFF" })] }),
        ],
      }),
      new TableCell({
        shading: { fill:C.HEADER_BG, type:ShadingType.CLEAR },
...
// 2A: バグ行 = 行7 (If a Mod 2 = 1 Then → 0)
const CODE_LINES = [
  { no:"1",  code:"Sub DisplayEven()" },
  ...
  { no:"8",  code:"        If a Mod 2 = 1 Then", bug:true },
...
function codeTable() {
  ...
  return new Table({
    ...
    rows: [
      ...
      ...CODE_LINES.map(({ no, code, bug })=>new TableRow({
        children: [
...
function specTable() {
  ...
  return new Table({
    ...
    rows:[
      new TableRow({ children:[hcell("セル", col), hcell("表示値（偶数のみ）", col)] }),
      ...["A1:2","A2:4","A3:6","A4:8","A5:10"].map(s=>{ const [c,v]=s.split(":"); return new TableRow({ children:[cell(c,{w:col}),cell(v,{w:col})] }); }),
    ],
  });
}
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{ width:PAGE_W, height:16838 }, margin:{ top:MARGIN, right:MARGIN, bottom:MARGIN, left:MARGIN } } },
  children:[
    titleBlock("26-06-15 条件判定 2A", "偶数表示（If文・Mod演算子）"),
    ...emptyLines(1),
    sectionHeader("状況説明"),
    para("太郎さんは「1 〜 n の中から偶数だけを A列に取り出して表示する」プログラムを作りました。", { indent:200 }),
    para("しかし実行してみると、偶数ではなく奇数が表示されてしまいます。", { indent:200 }),
    para("グループで原因を調べて、正しいプログラムに直してみましょう。", { indent:200, bold:true }),
    ...emptyLines(1),
    sectionHeader("プログラムの仕様"),
    para("①  ユーザーに「n（何まで調べるか）」を入力してもらう", { indent:200 }),
    para("②  1 〜 n の中から偶数だけを A列に上から順に表示する", { indent:200 }),
    ...
    para("何行目が間違っていますか？　なぜそうなるのか、Mod演算子の意味を含めて説明しましょう。", { indent:200 }),
    ...emptyLines(1),
    answerBox(4),
    ...emptyLines(1),
...
const outPath = path.join(__dirname,"..","worksheets","step2","2a_even-numbers.docx");
